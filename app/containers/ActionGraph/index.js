...
export class ActionGraph extends React.PureComponent { // eslint-disable-line react/prefer-stateless-function
  render() {
    ...
    if (!graph.sync) {
      ...
      const transform="scale(1 1) rotate(0) translate(" + (points[0][0] + 4) + " " + (points[2][1] + 4) + ")"
      const polyPoints =
        (points[0][0] - 4) + "," + (-points[0][1] + 4) + " " +
        (points[1][0] - 4) + "," + (-points[1][1] - 4) + " " +
        (points[2][0] + 4) + "," + (-points[2][1] - 4) + " " +
        (points[3][0] + 4) + "," + (-points[3][1] + 4) + " " +
        (points[0][0] - 4) + "," + (-points[0][1] + 4)
      console.log(drawP);
      return (
        <div>
            ...
            <g id="graph0" className="graph" transform={transform}>
              ...
              {g.objects.map((obj) => {
                ...
                return (<g id={"node" + obj.name} key={obj.name} className="node">
                  ...
                  <ellipse fill="none" stroke={drawC.color} cx={drawE.rect[0]} cy={-drawE.rect[1]} rx={drawE.rect[2]}
                           ry={drawE.rect[3]}/>
                  <text textAnchor="middle" x={lDrawT.pt[0]} y={-lDrawT.pt[1]} fontFamily={lDrawF.face}
                        fontSize={lDrawF.size} fill={lDrawC.color}>{obj.label}</text>
                </g>)
              })}
            </g>
          </svg>
